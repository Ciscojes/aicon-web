  ...
  return (
    <div className="public-shell">
      ...
      <main>
        ...
        <section className="public-section" id="condominios"><div className="public-section-heading"><div><p className="eyebrow">Nuestros proyectos</p><h2>Condominios para una nueva etapa.</h2></div><Link className="text-link" href="/catalogo">Ver todo el catálogo →</Link></div><div className="condominium-grid">{condominiums.map((item, index) => <article className={`condominium-card${item.coverImage ? " condominium-card-photo" : ""}`} key={item.id}>{item.coverImage ? <div aria-label={item.coverImage.altText} className="condominium-card-image" role="img" style={{ backgroundImage: `url("${item.coverImage.url}")` }} /> : null}<div className="condominium-card-content"><span className="condominium-number">0{index + 1}</span><p>{item.address || "Ubicación por confirmar"}</p><h3>{item.name}</h3><p>{item.description || "Conoce las casas disponibles en este proyecto."}</p><Link href={`/condominios/${item.slug}`}>Explorar proyecto →</Link></div></article>)}</div></section>
