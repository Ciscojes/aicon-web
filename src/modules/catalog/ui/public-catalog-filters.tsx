...
export function PublicCatalogFilters({ condominiums, query }: Readonly<{ condominiums: PublicCondominium[]; query: Record<string, string | string[] | undefined> }>) {
  ...
  return (
    <form action="/catalogo" className="public-filter-form" method="get">
      <div className="field"><label htmlFor="filter-condominium">Condominio</label><select defaultValue={value("condominio") ?? ""} id="filter-condominium" name="condominio"><option value="">Todos</option>{condominiums.map((item) => <option key={item.id} value={item.slug}>{formatPublicDisplayText(item.name)}</option>)}</select></div>
